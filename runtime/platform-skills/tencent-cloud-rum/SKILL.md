...
RUM2 MCP 已由 OJ 平台配置。直接使用本 Skill 所列工具，不要安装客户端、读取环境变量或尝试获取鉴权信息。
...
- 没有数据 → 扩大时间范围重试
- 鉴权失败 → 明确报告 RUM2 MCP 鉴权失败，由平台管理员检查服务配置
- 未指定时间 → 使用工具默认值
